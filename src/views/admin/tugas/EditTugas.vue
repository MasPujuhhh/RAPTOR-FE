          ...
          <div class="mb-3 row">
            <div class="col-md-8">
                ...
                <div class="col-md-12">
                  ...
                  <div v-for="(sub, index) in tugas.sub_tugas" :key="index" class="subtask-item mb-3">
                    ...
                    <div class="subtask-details d-flex w-100">
                      ...
                      <button class="btn btn-danger" @click="removeSubtask(index)">Remove</button>
                    </div>
                  </div>
                  <button class="btn btn-secondary" @click="addSubtask">Add Subtask</button>
                </div>
              </div>
                <div class="row mb-3">
                  <div class="col-md-2">
              ...
              <!-- <div class="row mb-3">
                <div class="col-md-12">
                  <h5>Sub Tugas</h5>
                  <div v-for="(sub, index) in tugas.sub_tugas" :key="index" class="d-flex mb-2">
                    <h3>{{ index + 1 }}.</h3>
                    <input type="text" id="subNamaTugas" class="form-control me-2" v-model="sub.judul" placeholder="Judul Sub Tugas">
                    <select id="addCategory" class="form-select me-2" v-model="sub.category_id">
                        <option disabled>Pilih Kategori</option>
                        <option v-for="kategori in kategori_children_option" :key="kategori.id" :value="kategori.id">
                          {{ kategori.deskripsi }}
                        </option>
                    </select>
                    <input type="date" class="form-control me-2" id="addTenggat" v-model="sub.tanggal_selesai">
                    <select id="addCategory" class="form-select me-2" v-model="sub.user_id">
                        <option disabled>Pilih User</option>
                        <option v-for="user in tugas.users" :key="user.id" :value="user.id">
                          {{ user.nama_lengkap }} ({{ user.nama_role }})
                        </option>
            ...
            <div class="col-md-4 rounded border border-1" style="padding:1rem">
              <h5>Ativitas</h5>
              <hr>
              <div class="row mb-3" style="padding-left:1rem;padding-right: 1rem;">
                <div class="col-md-12 px-2">
                  <h6 style="padding-bottom: 0; padding-top: 0.1rem;"><i class="bi bi-chevron-down"></i>&nbspLihat lebih banyak</h6>
                  <li>ashdjhb</li>
                </div>
              </div>
            </div>
...
const show_tags = computed(() => {
  return tugas.value.tags?.map(tag => `<span class="badge me-1" style="background-color:${tag.color}; color:black; margin-bottom: 1rem;"> <i class="bi bi-tag"></i> ${tag.nama}</span>`).join('');
});
...
const show_user = computed(() => {
  return tugas.value.users?.map(user => `
    <img src="${endpoint}${user.foto_profile}"  class="profile-pic" alt="..." style="width: 3rem; height: 3rem; object-fit: cover; margin-top: 20px; border-radius: 50%; margin-bottom:1rem" alt="Avatar" title="${user.nama_lengkap}">
  `).join('');
});
