...
<template>
    <div class="container-fluid d-flex flex-column align-items-center min-vh-100" style="margin-top: 5rem;">
        <Navbar></Navbar>
        <div class="row bg-success text-white rounded-4 p-2 w-100 d-flex justify-content-center align-items-center text-center mb-4"
            v-if="!show">
            <div class="col">
                <h1>ini notif</h1>
            </div>
        ...
        <div class="row w-100 mb-5">
            <div class="col p-3 d-flex justify-content-between align-items-center rounded-4 bg-white shadow">
                ...
                <h3>{{ moment(date).format('LL') !== 'no date' ? `${moment(date).format('dddd')},
        ...
        <div class="row d-flex w-100 min-vh-100">
            <div class="col col-4 d-flex justify-content-center p-3 h-25">
                <div class="d-flex flex-column align-items-center bg-white w-100 p-4 rounded-3 shadow">
                    ...
                    <div class="w-100">
                        <p class="d-flex justify-content-between fw-semibold">BAJ WMS FRONTEND <span
                                class="badge rounded-pill bg-secondary">{{ tasks.length }}</span></p>
                        <ul class="list-group">
                            <li class="list-group-item d-flex justify-content-between" v-for="task in tasks">
                                <span :class="task.is_check ? `text-decoration-line-through` : ``"><input
                                        type="checkbox" class="form-check-input me-2"
                                        @change="task.is_check = !task.is_check" style="cursor: pointer;"> {{ task.name }}</span><span
                                    v-if="task.is_check"><i class="bi bi-check-circle-fill"></i></span>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="col col-8 d-flex justify-content-center p-3">
                <div class="d-flex flex-wrap justify-content-center bg-secondary w-100 p-4 rounded-3 shadow gap-3"
                    ...
                    <div class="bg-white d-flex flex-column justify-content-between shadow rounded-3 p-3"
                        style="width: 45%; min-height: 25%;" v-for="task in tasks">
                    ...
                    <button class="bg-white shadow rounded-3 border-0 btn fs-1 "
                        style="width: 45%; min-height: 25%;">+</button>
                </div>
            </div>
        </div>
    </div>
