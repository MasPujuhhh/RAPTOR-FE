...
" class="container-content">
    ...
    <div class="w-100 d-flex justify-content-center align-items-center bg-primary mb-4" v-if="!show">
      <h1>waw</h1>
    </div>
    ...
    <div class="header mb-4">
      <div class="date">
        ...
        <h2 style="width:25%">{{ moment(date).format('LL') !== 'Invalid Date' ? `${moment(date).format('dddd')},
    ...
    <div class="flex-container">
      <div style="width: 30%;">
        <div class="left-content bg-secondary">
          <div class="todo">
            ...
            <div class="title">
              <div class="todo-head"
                ...
                <h4>TODO</h4>
                <h6> {{ todo ? todo[0]?.due_date ? `Tenggat, ${moment(todo[0]?.due_date).format('LL')}` : `tidak ada
                  tenggat` : '' }} </h6>
              </div>
            </div>
            <div>
            ...
            <div class="progress mb-4">
              ...
              <div class="progress-bar progress-bar-striped bg-success" role="progressbar"
                :style="todo ? todo[0]?.persen_progress : ''" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">
              </div>
            </div>
            <div class="todo-body">
              <!-- {{ todo }} -->
              ...
              <ol v-if="todo ? todo.length : ''" class="list-group list-group-numbered">
                <li v-for="list in todo[0]?.todo"
                  ...
                  <div v-if="!list.done" class="ms-2 me-auto" @click="simpanListId(list)" data-bs-toggle="modal"
                    :data-bs-target="'#modalCheck' + list.id">{{ list.description }}</div>

                  <div class="modal fade" :id="'modalCheck' + list.id" aria-labelledby="exampleModalLabel"
                    aria-hidden="true">
                      ...
                      <div class="modal-content">
                        ...
                        <div class="modal-footer">
                          ...
                          <button type="button" class="btn btn-primary" data-bs-dismiss="modal"
                            @click="checklist(list_id?.list_id)">
                            Konfirmasi
                          </button>
                        </div>
                      </div>
      ...
      <div class="right-content">
        <!-- {{ logbook ? logbook[0] : '' }} -->
        <div class="title">
          <div class="todo-head" style="margin-bottom: 1rem; padding: 0.5rem; border-bottom: 2px solid black;">
            <h4>LOGBOOK</h4>
          </div>
          <div v-if="logbook ? logbook[0]?.status == 0 : ''"
            style="width: 60%; display: flex; justify-content: space-between; font-size: 16px; padding: 0rem 1rem 0 1rem;">
            <p>Keterangan :
            </p>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1"
                v-model="keterangan" value="1">
              <label class="form-check-label" for="inlineRadio1">Masuk</label>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio2"
                v-model="keterangan" value="0">
              <label class="form-check-label" for="inlineRadio2">Ijin/Sakit</label>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio3"
                v-model="keterangan" value="">
              <label class="form-check-label" for="inlineRadio3">Tidak Masuk</label>
            </div>
          </div>

          <div v-if="logbook ? logbook[0]?.status == 1 : ''"
            style="display: flex; justify-content: space-between; font-size: 14px; padding: 0rem 1rem 0 1rem;">
        ...
        <div class="textarea" style="width: 100%;">
          <textarea v-if="logbook ? logbook[0]?.status == 0 : ''" v-model="textAreaValue"
            placeholder="Masukkan teks di sini" style="font-size: 1.2rem; padding: 1rem; height: 400px;"></textarea>
          <textarea v-if="logbook ? logbook[0]?.status == 1 : ''" v-model="logbook[0].description"
            :placeholder="logbook ? logbook[0].description : ''"
            style="font-size: 1.2rem; padding: 1rem; height: 400px;" disabled></textarea>
          <textarea v-if="logbook ? logbook[0]?.status == 2 : ''" v-model="logbook[0].description"
            style="font-size: 1.2rem; padding: 1rem; height: 400px;"></textarea>
          <textarea v-if="logbook ? logbook[0]?.status == 3 : ''" v-model="logbook[0].description"
            :placeholder="logbook ? logbook[0].description : ''"
            style="font-size: 1.2rem; padding: 1rem; height: 400px;" disabled></textarea>
          ...
          <div v-if="logbook ? logbook[0]?.status == 2 : ''">
            <h6>Komentar</h6>
            <textarea v-model="commentAreaValue" :placeholder="logbook[0]?.comment ? logbook[0]?.comment : '-'"
              style="padding: 0.5rem;" disabled></textarea>
          </div>
        ...
        <div style="display: flex; justify-content: end;">
          <button v-if="logbook ? logbook[0]?.status == 0 : ''" @click="kirim(logbook[0].id)" type="button"
            class="btn btn-success">
            Kirim
          </button>

          <button v-if="logbook ? logbook[0]?.status == 1 : ''" @click="" type="button" class="btn btn-success"
            disabled>
            Menunggu Persetujuan
          </button>

          <button v-if="logbook ? logbook[0]?.status == 2 : ''" @click="kirim(logbook[0].id)" type="button"
            class="btn btn-success">
            Kirim Ulang
          </button>
        </div>
