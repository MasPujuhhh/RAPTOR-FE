...
<template>
  <div class="container-fluid d-flex flex-column align-items-center min-vh-100" style="margin-top: 5rem">
    <Navbar></Navbar>
    <!-- PENGUMUMANNN -->
    <div class="row bg-warning text-white rounded-4 p-2 w-100 d-flex justify-content-between mb-4" v-if="unread_pengumuman?.length >= 1 && show_pengumuman">
      <div class="d-flex justify-content-between">      
        ...
        <div> 
          <h5 class="text-end" style="cursor:pointer" @click="show_pengumuman = !show_pengumuman"><i class="bi bi-x-lg"></i></h5>
    ...
    <div class="row w-100 mb-5">
      <div class="col p-3 d-flex justify-content-between align-items-center rounded-4 bg-white shadow">
        ...
        <h3>
          ...
          <button v-if="absensi?.keterangan == 'belum-absen-pagi'" @click="navigateAbsen(absensi.jadwal, 'check_in')" class="btn btn-sm btn-danger"><i class="bi bi-dash-circle" style="margin-right: 0.31rem;"></i>Check In</button>
          <button v-if="absensi?.keterangan == 'belum-absen-sore'" @click="navigateAbsen(absensi.jadwal, 'check_out')" class="btn btn-sm btn-danger"><i class="bi bi-dash-circle" style="margin-right: 0.31rem;"></i>Check Out</button>
    ...
    <div class="row d-flex w-100 min-vh-100">
      <div class="col col-4 d-flex justify-content-center p-3 h-25">
        <div class="d-flex flex-column align-items-center bg-white w-100 p-4 rounded-3 shadow">
          ...
          {{ tugas?.judul}}
          <h6 v-if="tugas_id && tugas_id != 'aRh0ymVac_gmST2UGkHTy'" class="w-100" style="text-align: left;">progress bar {{ `${tugas_detail?.sub_tugas_progress}` }}</h6>
          ...
          <div v-if="tugas_id" class="w-100">
            ...
            <ul class="list-group">
              <li class="list-group-item d-flex justify-content-between" v-for="sub in sub_tugas">
                <span>
                  <input v-if="me.id == sub.user_id" type="checkbox" class="form-check-input me-2" data-bs-toggle="modal" data-bs-target="#exampleModal" style="cursor: pointer;" :checked="sub.is_done" :disabled="sub.is_done" @change="getDetailSubTugas(sub, $event)">
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <div> 
                      <h5 class="modal-title" id="exampleModalLabel">Apakah Yakin Menyelesaikan Tugas {{ sub_tugas_detail?.judul }}</h5>
                  ...
                  <div v-if="cekTerlambat(new Date(sub_tugas_detail?.tanggal_selesai), new Date())" class="modal-body">
                    ...
                    <div>
                      <textarea type="text" class="form-control" id="filterNamaLengkap" v-model="alasan_terlambat" placeholder="Alasan Terlambat"/>
                      <p v-if="cekTerlambat(new Date(sub_tugas_detail?.tanggal_selesai), new Date()) && !alasan_terlambat" class="text-danger">Silahkan isi alasan keterlambatan</p>
                    </div>
                  </div>
                  <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
                    <button type="button" class="btn btn-primary" @click="doneSubTugas(sub_tugas_detail?.sub_tugas_id)" :disabled="cekTerlambat(new Date(sub_tugas_detail?.tanggal_selesai), new Date()) && !alasan_terlambat">Simpan</button>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div class="col-md-12 px-1">
                  <div v-if="!show_history">
                    ...
                    <ul class="list-group list-group-flush">
                      <li class="list-group-item">{{ (tugas_detail?.histories?.[tugas_detail.histories.length -1])?.keterangan }}</li>
                    </ul>
                  </div>
                  <div v-else>
                    ...
                    <ul class="list-group list-group-flush">
                      <li  v-for="history in tugas_detail.histories" :key="history.id" class="list-group-item">{{ history.keterangan }}</li>
                    </ul>
                  </div>
                </div>
              </div>
      ...
      <div class="col col-8 d-flex justify-content-center align-items-start p-3">
        <div class="d-flex flex-wrap justify-content-center bg-secondary w-100 p-4 rounded-3 shadow gap-3" style="overflow-y: scroll">
          <div  class="bg-white d-flex flex-column justify-content-between shadow rounded-3 p-3" style="width: 45%; min-height: 300px; max-height: 350px" v-for="(report, index) in daily_report" :key="index">
            ...
            <template v-else>
              ...
              <div class="d-flex gap-1 justify-content-between border-top border-black pt-2">
                <img :src="`${endpoint}${report?.foto_profile}`" class="profile-pic" alt="..." style="width: 2.5rem; height: 2.5rem; object-fit: cover; border-radius: 50%; margin-right: 1rem;" :alt="Avatar" :title="report?.nama_lengkap">
                <h6>{{ report?.judul }}</h6>
                <h6>
                  <span class="badge" :style="`background-color:${report?.color}; color:black;`"><i class="bi bi-bookmark-fill"></i>{{ report?.nama_label }}</span>
                </h6>
          ...
          <div
            ...
            style="width: 45%; min-height: 300px; max-height: 350px"
            v-if="showtask"
          >
            <div class="h-50">
              <textarea v-model="new_report.deskripsi" type="text" class="form-control h-100" placeholder="Deskripsi Daily Report" />
            </div>
            <div class="d-flex gap-2 justify-content-center border-top border-black py-1 h-25">
              <div class="d-flex h-75 gap-4" style="width: 100%">
                ...
                <select v-model="new_report.label_id" class="form-select form-select-sm">
                  ...
                  <option v-for="label in labels" :key="label.id" :value="label.id" :style="`background-color:${label.color}`"><i class="bi bi-bookmark-fill"></i>{{ label.nama }}</option>
                </select>
              </div>
            </div>
            <div
